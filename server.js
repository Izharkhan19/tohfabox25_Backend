...
const { URL } = require("./config/envConfig"); // Import CORS URL
...
// --- API Routes ---
app.use("/api/auth", authRoutes); // Authentication routes (e.g., /api/auth/signup, /api/auth/signin)
app.use("/api", mediaRoutes);    // Media routes (e.g., /api/upload, /api/media, /api/delete)
...
app.listen(port, () => {
  console.log(`Server running on : ${port}`);
  console.log(
    "Ensure CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, CLOUDINARY_API_SECRET, JWT_SECRET, and MONGODB_URI are set in your .env file."
  );
});
